...
# load in the data
ncha <- read.csv(file = "Onramp/Data/NCHA2021_Session3_5_Subset.csv", header = TRUE, na.strings=c("","NA"))
...
str(NCHA2021_Session3_5_Subset)
...
ggplot(ncha) + geom_boxplot(aes(x = AGE))
...
ggplot(ncha) + geom_boxplot(aes(x = OVRL_HEALTH, y = MIN_MOD_PA))
...
ggplot(ncha) + geom_point(aes(x = AGE, y = MIN_MOD_PA), position = "jitter")
...
ggplot(ncha) + geom_point(aes(x = AGE, y = MIN_MOD_PA, color = OVRL_HEALTH), position = "jitter") +
  ylab("Moderate Physical Acitivity (minutes)") + 
  xlab("Age (years)") + 
  labs(color = "Overall Health") +
  ggtitle("Moderate Physical Activity and Age") + 
  theme_light() + 
